...
Given('用户进入 智脑 对话页面', { timeout: 30_000 }, async function (this: CustomWorld) {
  ...
  console.log('   📍 Step: 查找 智脑...');
  // Find and click on "智脑" agent in the sidebar/home
  const lobeAIAgent = this.page.locator('text=智脑').first();
  await expect(lobeAIAgent).toBeVisible({ timeout: WAIT_TIMEOUT });

  console.log('   📍 Step: 点击 智脑...');
...
Given('用户已发送消息 {string}', async function (this: CustomWorld, message: string) {
  ...
  // Assistant messages are left-aligned .message-wrapper elements that contain "智脑" title
...
Then('回复内容应该可见', async function (this: CustomWorld) {
  const assistantMessage = this.page.locator('.message-wrapper').filter({
    has: this.page.locator('.message-header', { hasText: /智脑|AI/ }),
  });
      ...
      async () => {
        const rawText =
          (await assistantMessage
            ...
            .innerText()
            .catch(() => '')) || '';
        finalText = rawText.replaceAll('智脑', '').replaceAll(/[·•]/g, '').trim();
